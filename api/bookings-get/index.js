const { bookings, dayRangeUtc } = require('../lib/cosmos.js');
...
module.exports = async function (context, req) {
  ...
  if (email) {
    const q = {
      query: 'SELECT * FROM c WHERE LOWER(c.requesterEmail)=@e',
  ...
  if (!date) {
    context.res = { status: 400, body: 'date=YYYY-MM-DD required (or email=)' };
    return;
  }
  ...
  if (roomId) {
    where += ' AND c.roomId = @room';
    params.push({ name: '@room', value: roomId });
  }

  const { resources } = await bookings.items
    .query({ query: `SELECT * FROM c ${where}`, parameters: params })
    .fetchAll();
